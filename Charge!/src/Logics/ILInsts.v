...
(*
Require Import Open Stack Equiv.

Section ILOpen.
  Context {B : Type} `{HIL : ILogic B}.
  Context {A : Type} `{Heq : DecidableEq A}.

  Ltac reverse_lift_aux' t :=
    match t with
      | ?s ?x =>
        match type of s with
          | stack A => constr:(var_expr x)
        end
      | ?f ?x => 
        let lf := reverse_lift_aux' f in
        let lx := reverse_lift_aux' x in
        constr:(lf lx)
      | ?x => constr:(@liftn A _ _ _ _ _ x)
    end.

  Ltac reverse_lift_aux t :=
    first [has_evar t | reverse_lift_aux' t].
    ...
    match goal with
      | s : stack A |- _ =>
        match goal with
          | |- ?x |-- ?y =>
            let lx := reverse_lift_aux x in
            let ly := reverse_lift_aux y in 
            replace x with (lx s) by reflexivity;
              replace y with (ly s) by reflexivity;
              apply ILFun_fold_entails
