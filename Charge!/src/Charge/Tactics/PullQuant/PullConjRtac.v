...
Section parametric.
  ...
  Definition lem_conj_sym (l : typ)
  : rw_lemma typ func (expr typ func) :=
  {| Lemma.vars := l :: l :: nil
   ; Lemma.premises := nil
   ; Lemma.concl := {| lhs := mkAnd l (Var 0) (Var 1);
                       rel := Rinj (Inj (fEntails l));
                       rhs:= mkAnd l (Var 1) (Var 0) |}
  |}.
                                        
  Definition lem_conj_ass1 (l : typ)
  : rw_lemma typ func (expr typ func) :=
  {| Lemma.vars := l :: l :: l :: nil
   ; Lemma.premises := nil
   ; Lemma.concl := {| lhs := mkAnd l (mkAnd l (Var 0) (Var 1)) (Var 2);
                       rel := Rinj (Inj (fEntails l));
                       rhs:= mkAnd l (Var 0) (mkAnd l (Var 1) (Var 2)) |}
  |}.
                                        
  Definition lem_conj_ass2 (l : typ)
  : rw_lemma typ func (expr typ func) :=
  {| Lemma.vars := l :: l :: l :: nil
   ; Lemma.premises := nil
   ; Lemma.concl := {| lhs := mkAnd l (Var 0) (mkAnd l (Var 1) (Var 2));
                       rel := Rinj (Inj (fEntails l));
                       rhs:= mkAnd l (Var 1) (mkAnd l (Var 0) (Var 2)) |}
  |}.
  ...
  Variable filter : expr typ func -> bool.

  Definition the_lemmas (e : expr typ func) (_ : R typ Rbase)
  : list (rw_lemma typ func (expr typ func) *
          CoreK.rtacK typ (expr typ func)) :=
    poly_apply (por (pmap (fun t_a_b => 
                        let '(t, a, b) := t_a_b in
                        if andb (negb (filter a)) (filter b) then
                              (lem_conj_sym t, IDTACK) :: nil
                            else 
                              nil) (ptrnAnd get get get))
                    (por (pmap (fun t_r => 
                                  let '(t, _, _) := t_r in
                                  (lem_conj_ass1 t, IDTACK) :: nil)
                               (ptrnAnd get (ptrnAnd ignore ignore ignore) ignore))
                         (pmap (fun t_a_b =>
                               let '(t, a, (_, b, _)) := t_a_b in
                               if andb (negb (filter a)) (filter b) then 
                                 (lem_conj_ass2 t, IDTACK)::nil
                               else
                                 nil)
                               (ptrnAnd get get (ptrnAnd ignore get ignore))))) e.
...
*)
  ...
  Theorem pull_all_conj_sound
  : setoid_rewrite_spec RbaseD pull_all_conj.
  Proof.
